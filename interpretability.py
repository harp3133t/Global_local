...
# pretrained model
checkpoint_path = '/raid/harp3133t/FG_regularization_with_stprotopnet/saved_models/20240710_1435/vgg19/001/29nopush0.7801.pth'
...
GT_PATH = '../data_legacy/segmentations'
...
for i, (img_name, label) in enumerate(test_dataset.imgs):
    ...
    heatmap_deletion = (heatmap_deletion - heatmap_deletion.min()) / ((heatmap_deletion.max() - heatmap_deletion.min()) + epsilon)
    ...
    oirr = (heatmap[gt_mask == 0].sum() / (gt_mask == 0).sum()) / ((heatmap[gt_mask == 1].sum() / (gt_mask == 1).sum()) + epsilon)
